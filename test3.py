...
if __name__ == "__main__":
    ...
    tok_sep = "#"
    pts_dir = Path("/home/durgai/CTC/pts/logits")
    ...
    for i, filepath in enumerate(tqdm(filepaths, leave=False)):
        logits, _, _ = torch.load(filepath)
        logits = logits.unsqueeze(0)
        seq_lens = torch.empty((1), dtype=torch.int32).fill_(logits.shape[-2])
        ...
        for op_s_old, op_s_new, s_l, a_max in zip(
            ...
            if not torch.equal(op_s_old, a_max):
                OLD_err += 1
            if not torch.equal(op_s_new, a_max):
                NEW_err += 1
            # for old_label, new_label, a_value in zip(op_s_old, op_s_new, a_max):
            #     if (new_label != a_value): NEW_err += 1

            #     elif (old_label != a_value):  OLD_err += 1
